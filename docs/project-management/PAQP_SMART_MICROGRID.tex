...
\hline
Rapport SonarQube & v1.0 & RT & ⏳ Avant 23/12/2025 \\
\hline
...
\begin{itemize}[leftmargin=*]
    ...
    \item Critère : Temps réponse < 1s (95e percentile)
    \item Responsable : RT
    \item \textbf{Justification} : Pour un projet étudiant ciblant des établissements médicaux (probablement < 50 utilisateurs réels), 20-30 users simultanés est un objectif réaliste et suffisant.
\end{itemize}
...
\hline
DEV (Local) & Postes développeurs, tests locaux \\
\hline
INT (Integration) & Serveur intégration, Docker Compose, CI/CD \\
\hline
UAT (Pre-prod) & Environnement similaire production, tests acceptance \\
\hline
PROD (Production) & Production finale (si déploiement réel demandé) \\
\hline
\end{tabular}
...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{Notification} → Équipe (Slack/Email) en cas d'échec
\end{enumerate}
...
\hline
Dette technique & Jours équivalents refactoring & < 5 jours \\
\hline
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Livrable} : Rapport audit sprint
\end{itemize}
...
\begin{itemize}[leftmargin=*]
    \item \textbf{Analyse statique} : SonarQube / SonarCloud (si disponible)
    \item \textbf{Linters} :
    \begin{itemize}
