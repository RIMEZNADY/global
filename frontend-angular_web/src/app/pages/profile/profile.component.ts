...
export class ProfileComponent implements OnInit, OnDestroy {
    ...
    this.apiService.get<UserProfile>('/auth/me').subscribe({
      next: (profile) => {
        ...
        if (!profile.firstName || !profile.lastName) {
          console.warn('WARNING: Profile missing firstName or lastName!');
      ...
      error: (error) => {
        ...
        if (error.status === 0) {
          ...
          console.warn('Token invalid or expired (401/403)');
  ...
  initForm(): void {
    if (!this.userProfile) {
      console.warn('initForm called but userProfile is null!');
      this.initEmptyForm();
      return;
    }
