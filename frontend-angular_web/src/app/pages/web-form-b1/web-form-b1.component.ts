...
export class WebFormB1Component implements OnInit, AfterViewInit {
  ...
  onMapReady(map: L.Map): void {
    ...
    // Si on a déjà une position sauvegardée, afficher le marqueur et la zone
    if (this.lat != null && this.lng != null && this.irradiationClass) {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        if (lat >= 20 && lat <= 36 && lng >= -17 && lng <= -1) {
          ...
          // Position hors du Maroc - pas d'erreur, juste ne pas utiliser la position
          this.isLoadingLocation = false;
        }
      },
      (err) => {
        ...
        // Pas de message d'erreur - l'utilisateur peut sélectionner sur la carte
      },
